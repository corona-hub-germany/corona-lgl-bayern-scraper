...
module.exports = class LGLBayernCoronaDataScraper {
  ...
  async loadPage() {
    ...
    if (this.options.useFileCache && fs.existsSync(CACHE_FILE)) {
      ...
      try {
        ...
        throw new Error(
          `Error requesting remote url "${this.options.url}": ${err}`
        );
      }
  ...
  getCopyright(options) {
    ...
    if (!text || !text.length) {
      throw new Error(
        `Could not find selector "${opts.textSepector}" in html document`
      );
    }

    return [text, "© Statistisches Bundesamt (Destatis)"];
  }
  ...
  getLastUpdate(options) {
    const opts = Object.assign(
      {
        captionSepector:
          "#content_1c > div.abstand_unten > div > table > caption",
    ...
    if (!caption || !caption.length) {
      throw new Error(
        `Could not find selector "${opts.captionSepector}" in html document`
      );
    }
    ...
    if (!dateString || !dateString.length) {
      throw new Error(
        `Could not find datestring "${opts.parseRegex}" in "${caption}"`
      );
    }
    ...
    if (!dateTime.isValid()) {
      throw new Error(
        `Could not parse date-time from "${dateString}" using format "${opts.dateTimeFormat}"`
      );
    }
  ...
  getTableEntries() {
    const $rows = this.$(
      "#content_1c > div.abstand_unten > div > table > tbody > tr"
    );
    ...
    $rows.each((index, element) => {
      ...
      if (columnCount !== 7) {
        throw new Error(`Invalid column count "${columnCount}" expected "7"`);
			...
			var district;
			var districtName
      if (name.includes("Stadt")) {
        district = getDistrictByNameAndType(
          name.replace("Stadt", "").trim(),
          "Kreisfreie Stadt"
				);
				districtName = `${district.name} (Stadt)`;
      } else {
				district = getDistrictByNameAndType(name, "Landkreis");
      ...
      const entry = {
        ...
        population_female: district.population_female,
        population_per_km2: district.population_per_km2,
        ...
        infected_indicator_100k: this._getNumberFromString(
          infected_indicator_100k
        ),
